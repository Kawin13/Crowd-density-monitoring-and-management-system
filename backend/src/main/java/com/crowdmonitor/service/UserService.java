...
public class UserService {
    ...
    public void deleteUser(Long id) {
        ...
        try {
            ...
            // audit_logs.user_id, alerts.acknowledged_by, and reports.generated_by
            // all reference users(id) with no ON DELETE CASCADE (by design —
            // audit/report history should outlive the user). Almost any real
            // user will have at least one audit log entry (e.g. their own
            // login), so this path is common, not an edge case. Surface it as
            // a clear 400 instead of letting the raw FK violation fall through
            // to a generic 500.
            throw new IllegalArgumentException(
                    "Cannot delete user '" + username + "': they have existing activity history " +
                    "(audit logs, acknowledged alerts, or generated reports). Deactivate the account instead.");
        }
